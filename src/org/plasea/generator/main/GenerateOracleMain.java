...
import org.plasea.generator.database.oracle.OracleTableFactory;
 ...
 *
 */
public class GenerateOracleMain {
	...
	public static void generate(){
		...
		//读取配置文件，初始化变量
		new GenerateOracleMain().init();
		//首先判断，是根据数据库名还是根据单个表名来生成
		if(dataBaseName!=null&&!dataBaseName.equals("")){//根究数据库名来生成的
			List tableList=OracleTableFactory.getTablesByDataBase(dataBaseName);
			if(tableList!=null&&tableList.size()>0){
				tables=new String[tableList.size()];
				for (int i = 0; i < tableList.size(); i++) {
					Object[] obj= (Object[]) tableList.get(i);
					tables[i]=(String) obj[0];
				}
			}
		...
		for(int i=0;i<tables.length;i++){
			table= OracleTableFactory.generateTable(tables[i].trim());
