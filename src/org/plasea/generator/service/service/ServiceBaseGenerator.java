package org.plasea.generator.service.service;
 ...
 *
 */
public class ServiceBaseGenerator {
	...
	public static void generateServiceBase(GenerateInfo generateInfo,Table table){
		String packagePath = PathUtil.getPackagePath(generateInfo.getProjectPath(), generateInfo.getServiceBasePackage());
		
		Configuration cfg = new Configuration();  
        try {  
            cfg.setClassForTemplateLoading(ModelGenerator.class, "/org/plasea/generator/service/service");//指定模板所在的classpath目录  
            cfg.setSharedVariable("upperFC", new UpperFirstCharacter());//添加一个"宏"共享变量用来将属性名首字母大写  
            Template t = cfg.getTemplate("service-base.html");//指定模板  
	 ...
	 * @param table
	 */
	public static void generateServiceBase(GenerateInfo generateInfo,OracleTable table){
		String packagePath = PathUtil.getPackagePath(generateInfo.getProjectPath(), generateInfo.getServiceBasePackage());
